         ...
         ((slot-boundp obj 'transient)
          (define-key map (vector sym)
            (let ((do (oref obj transient)))
              (pcase do
                (`t (cond (sub-prefix #'transient--do-recurse)
                          ((cl-typep obj 'transient-infix)
                           #'transient--do-stay)
                          (t #'transient--do-call)))
                (`nil (if sub-prefix #'transient--do-replace
                        #'transient--do-exit))
                (_ do)))))
    ...
    (if-let ((value (oref obj value)))
        (propertize
         (pcase (oref obj multi-value)
           ...
           (`concat (mapconcat (lambda (v) (concat argument v)) value " "))
           (_       (concat argument (mapconcat #'identity value ","))))
