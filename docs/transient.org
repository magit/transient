...
#+texinfo: @noindent
This manual is for Transient version 0.13.7.
...
- ~claim-argument~ Whether to claim matched arguments exclusively, i.e.,
  whether to prevent other "overlapping" infixes from also matching.

  For example, consider the suffix arguments ~--push-option=known=<val>~
  and ~--push-option=<arbitrary>~.  Unless the former claims any matched
